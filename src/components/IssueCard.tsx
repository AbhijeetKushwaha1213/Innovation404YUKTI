...
const IssueCard: React.FC<IssueCardProps> = ({
  ...
  // Get priority level based on upvote count
  const priority = getPriorityLevel(currentUpvoteCount);
      ...
      <Link to={`/issues/${id}`} className="block">
        {imageArray.length > 0 && (
          ...
          >
            ...
            <div 
              className={cn(
                "absolute top-3 right-3 px-2 py-1 rounded-md text-xs font-semibold border flex items-center gap-1.5",
                priority.color
              )}
            >
              <span className={cn("w-2 h-2 rounded-full", priority.dotColor)}></span>
              {priority.label}
            </div>
        ...
        <div className="p-5">
          <div className="flex items-center justify-between mb-2">
            ...
            {/* Priority Badge for cards without images */}
            {!image && (
              <div 
                className={cn(
                  "inline-flex px-2 py-1 rounded-md text-xs font-semibold border items-center gap-1.5",
                  priority.color
                )}
              >
                <span className={cn("w-2 h-2 rounded-full", priority.dotColor)}></span>
